...
import type { ScreenIndex } from "../Components/AppScreen";
import type { availableTables } from "../constants/tables";
import type { ITurn } from "./game.utils";
...
const defaultAppState: IAppState = {
	screen: "home",
	tables: new Set<keyof typeof availableTables>([2, 3, 4, 5, 6, 7, 8, 9]),
	timer: 0,
	turn: null,
	score: 0,
	bestScore: 0,
...
const AppReducer = (state: IAppState, action: AppReducerAction): IAppState => {
	switch (action.type) {
		...
		case "ENABLE_TABLE": {
			const tables = new Set<keyof typeof availableTables>([
				...state.tables,
				action.payload,
			]);
			console.log(tables);
...
export const AppContextProvider = ({
	...
	const [context, dispatch] = useReducer<Reducer<IAppState, AppReducerAction>>(
		AppReducer,
		defaultAppState,
	);
